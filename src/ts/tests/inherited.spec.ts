...
[5, 20, 50, 500].forEach( (size: number) => {
  ...
  test(`Infinite plane straight line, with diagonal, size ${size}`, () => {
    ...
    for (let i=size; i>=0; --i) { expect(result1.path[i]).toStrictEqual([size-i,size-i]); }
...
[5, 20, 50].forEach( (size: number) => {
  ...
  test(`Infinite plane straight line, no diagonal, size ${size}`, () => {
