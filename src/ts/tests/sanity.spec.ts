...
test('Config with negative timeout', () => {
  expect(() => a_star({ start: 1, isEnd: () => false, neighbor: () => [], distance: () => 0, heuristic: () => 0, timeout: -1 } as any)).toThrow();
});

test('Config with fractional timeout', () => {
  expect(() => a_star({ start: 1, isEnd: () => false, neighbor: () => [], distance: () => 0, heuristic: () => 0, timeout: 1.5 } as any)).toThrow();
});

test('Config with string timeout', () => {
  expect(() => a_star({ start: 1, isEnd: () => false, neighbor: () => [], distance: () => 0, heuristic: () => 0, timeout: '2' } as any)).toThrow();
});
...
test('Immediate timeout', () => {
  expect(
    a_star({
      ...
      heuristic : ([x,   y]: [x:number, y:number]) => cartesian_2d.diagonal_distance([x,y], [ maze_tgt.end[0], maze_tgt.end[1] ]),
      timeout   : 1
    })
