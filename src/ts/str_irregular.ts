...
function from_edges(edgelist: StrIrregularList) {
  ...
  edgelist.forEach( edge => {
    ...
    if (mesh.has(key)) { throw new Error(`Duplicate: ${key}`); }
    else               { mesh.set(key, edge[2]); }
    ...
    if (s0 === undefined) { throw new Error('Impossible, just set s0 edge0') }
    else                  { if (!(s0.has(edge[1]))) { s0.add(edge[1]); } }

    const s1 = neighbor_lists.get(edge[1]);
    if (s1 === undefined) { throw new Error('Impossible, just set s1 edge1') }
    else                  { if (!(s1.has(edge[0]))) { s1.add(edge[0]); }     }
  ...
  return {
    ...
    distance : (l: string, r: string): number => {
      ...
      if (distance === undefined) { throw `No such edge ${l} : ${r}`; }
